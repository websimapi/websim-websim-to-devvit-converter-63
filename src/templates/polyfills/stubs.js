                ...
                return new Promise((resolve) => {
                        ...
                        innerHtml = \`
                            <div style="background:#1e293b;padding:24px;border-radius:12px;width:90%;max-width:400px;text-align:center;border:1px solid #334155;">
                                ...
                                <div style="display:flex;gap:10px;flex-direction:column;">
                                    <button id="ws-modal-tip" style="background:#FF4500;color:white;border:none;padding:12px 24px;border-radius:6px;font-weight:bold;cursor:pointer;width:100%;font-size:1.1rem;">Confirm & Pay \${data.credits} Gold</button>
                    ...
                    if (isTip) {
                        ...
                        modal.querySelector('#ws-modal-tip').onclick = async () => {
                            ...
                            try {
                                if (!window.purchase || !window.OrderResultStatus) {
                                ...
                                if (result.status === window.OrderResultStatus.STATUS_SUCCESS) {
                                    console.log("[Polyfill] Purchase Successful");
                                    close();
                                    resolve({});
                                } else {
                                    throw new Error(result.errorMessage || "Payment failed or was cancelled");
                                }
                            } catch (e) {
                                console.error("[Polyfill] Purchase Error:", e);
