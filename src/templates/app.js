...
// --- Devvit Web Payment Fulfillment ---
router.post('/internal/payments/fulfill', async (req, res) => {
    try {
        console.log('[Server] Payment Fulfillment Request Received');
        // The order object is typically passed in the body by the Reddit proxy
        const order = req.body;
        
        // Fulfillment logic is handled by addPaymentHandler below, 
        // but Web apps require a successful JSON response to the endpoint.
        res.json({ success: true } satisfies PaymentHandlerResponse);
    } catch (e) {
        console.error('[Server] Fulfillment Endpoint Error:', e);
        res.json({ success: false, reason: e.message } satisfies PaymentHandlerResponse);
    }
});
